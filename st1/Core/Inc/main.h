/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f1xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define st1_st2_TX_Pin GPIO_PIN_2
#define st1_st2_TX_GPIO_Port GPIOA
#define st1_st2_RX_Pin GPIO_PIN_3
#define st1_st2_RX_GPIO_Port GPIOA
#define motor1_in2_Pin GPIO_PIN_6
#define motor1_in2_GPIO_Port GPIOA
#define motor2_in2_Pin GPIO_PIN_7
#define motor2_in2_GPIO_Port GPIOA
#define motor3_in2_Pin GPIO_PIN_0
#define motor3_in2_GPIO_Port GPIOB
#define motor4_in2_Pin GPIO_PIN_1
#define motor4_in2_GPIO_Port GPIOB
#define HC05_TX_Pin GPIO_PIN_10
#define HC05_TX_GPIO_Port GPIOB
#define HC05_RX_Pin GPIO_PIN_11
#define HC05_RX_GPIO_Port GPIOB
#define motor1_in1_Pin GPIO_PIN_6
#define motor1_in1_GPIO_Port GPIOB
#define motor2_in1_Pin GPIO_PIN_7
#define motor2_in1_GPIO_Port GPIOB
#define motor3_in1_Pin GPIO_PIN_8
#define motor3_in1_GPIO_Port GPIOB
#define motor4_in1_Pin GPIO_PIN_9
#define motor4_in1_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
